...
export default function HirerLogin() {
  ...
  const handleSubmit = async e => {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/login-hirer', {
        method: 'POST',
  ...
  return (
    <form className="max-w-md mx-auto bg-white rounded-xl shadow-lg p-8 mt-10 relative" onSubmit={handleSubmit} autoComplete="off">
      ...
      <div className="mb-4">
        ...
        <input name="identifier" value={form.identifier} onChange={handleChange} className="w-full border rounded px-3 py-2" required />
      </div>
      <div className="mb-6">
